...
[Serializable]
public class BoundingBox : IEquatable<BoundingBox>, IBoundingShape, IIntersectShape
{
    ...
    {
       var TestCenter = aCenter;
        if (TestCenter != aCenter)
        {
            var offset = CalculateCenterOffset(Extents.Center, aCenter);
    ...
    public BoundingBox(float Length, float Width, float Height, Vector3 aCenter)
    {

        this.Center = aCenter ;
        this.Min = Vector3.Zero;
        this.Max = new Vector3(Length, Width, Height);        
        ...
        if (TestCenter != aCenter)
        {
            ...
            Logger.LogIf((TestCenter + offset) != aCenter, String.Format("Values for the center and test center offset are incorrect in Bounding Box"));
    ...
    public BoundingBox(Vector3 min, Vector3 max, Vector3 aCenter)
    {
        this.Min = min;
        this.Max = max;
        this.Center = aCenter;
    ...
    public BoundingBox(float MinValue, float MaxValue, Vector3 aCenter)
    {
        this.Center = aCenter;
        var aMin = new Vector3(MinValue);
        var aMax = new Vector3(MaxValue);
        var  TestCenter = ((aMax - aMin) * 0.5f) + aMin;
        if(TestCenter != aCenter)
        {
            var offset = CalculateCenterOffset(TestCenter, aCenter);
            this.Max = aMax + offset;
            this.Min = aMin + offset;
        }
        else
        {
            this.Max = aMax;
    ...
    public BoundingBox(BoundingBox bounds) : this(bounds.Min, bounds.Max)
    {
    ...
    public BoundingBox SetCenter(Vector3 aCenter)
    {
        var value = aCenter;
        if (this.Center != aCenter)
        {
            var Offset = CalculateCenterOffset(Center, value);
            this.Center += Offset;
            Min = Min + Offset;
            Max = Max + Offset;
        }
    ...
    private Vector3 CalculateCenterOffset(Vector3 CurrentCenter, Vector3 newCenter)
    {
        return Vector3.Subtract(newCenter, CurrentCenter);
    }

    public ContainmentType Contains(BoundingBox box)
    {
        //test if all corner is in the same side of a face by just checking min and max
        //TODO: IF ERROR THEN REMOVE =
        if (box.Max.X <= Min.X
            || box.Min.X >= Max.X
            || box.Max.Y <= Min.Y
            || box.Min.Y >=Max.Y
    ...
    private static bool Validate(BoundingBox a, BoundingBox b)
    {
    ...
    {
        return "{{Min:" + this.Min.ToString() + " Max:" + this.Max.ToString() + " Center: " + Center +"}}";
    }
